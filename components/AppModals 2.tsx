...
import { getStockStatus, StockMeter, Badge } from './Shared';
import { DeferredContentFallback } from './DeferredContentFallback';
import { AIAssistant, MobileStockCountView, StockTransferModal } from '../app/lazyViews';
...
export interface AppModalsState {
  ...
  setIsAdjustModalOpen: (open: boolean) => void;
  ...
  setIsEditContactModalOpen: (open: boolean) => void;
  setContactToEdit: (contact: Contact | null) => void;
  isDetailModalOpen: boolean;
  setIsDetailModalOpen: (open: boolean) => void;
  setItemToShow: (item: InventoryItem | null) => void;
  isAllocateModalOpen: boolean;
  setIsAllocateModalOpen: (open: boolean) => void;
  setJobToAllocate: (job: Job | null) => void;
  setAllocSearch: (val: string) => void;
  setAllocItemId: (val: string) => void;
  isTemplateModalOpen: boolean;
  setIsTemplateModalOpen: (open: boolean) => void;
  setTemplateJob: (job: Job | null) => void;
  setSelectedStockSourceName: (name: string) => void;
  setTemplateItems: (items: AllocatedItem[]) => void;
  showAIAssistant: boolean;
  setShowAIAssistant: (open: boolean) => void;
  isMobileStockCountOpen: boolean;
  setIsMobileStockCountOpen: (open: boolean) => void;
}

export interface AppModalsHandlers {
  handleConfirmPick: (jobId: string) => Promise<void>;
  handleImportCSV: (event: React.ChangeEvent<HTMLInputElement>) => void;
  handleDeleteItem: (item: InventoryItem) => Promise<void>;
  handleDeleteAllItems: () => Promise<void>;
  ...
  handleAddTemplate: (name: string, items: AllocatedItem[]) => Promise<void>;
  handleUpdateTemplate: (id: string, name: string, items: AllocatedItem[]) => Promise<void>;
  handleDeleteTemplate: (id: string) => Promise<void>;
  handleMobileStockUpdate: (itemId: string, newQuantity: number) => Promise<void>;
  getAllocatedItemsFromSelection: (selection: string) => AllocatedItem[];
  getStockSourceName: (selection: string) => string;
}

// ── Standalone hook that owns all modal state + handlers ─────────────────────

export interface UseAppModalsReturn {
  state: AppModalsState;
  handlers: AppModalsHandlers;
  modalsJSX: React.ReactNode;
}
...
export function useAppModals(
  ...
  const pickJob = useStore((s) => s.pickJob);
  ...
  const deleteAllInventoryItems = useStore((s) => s.deleteAllInventoryItems);
  ...
  const setInventoryState = useStore((s) => s.setInventoryState);
  const setJobsState = useStore((s) => s.setJobsState);
  const setMovementsState = useStore((s) => s.setMovementsState);
  ...
  const buildAllocatedItemsFromKit = (kit: Kit): AllocatedItem[] => {
    ...
    kit.items.forEach((kitItem) => {
      ...
      aggregatedItems.set(inventoryItemId, (aggregatedItems.get(inventoryItemId) || 0) + kitItem.quantity);
    });
  ...
  const getStockSourceName = (selection: string): string => {
    ...
    return templates.find((t) => t.id === selection.replace('template:', ''))?.name || 'Selected template';
  };
  ...
  const handleCreateJob = async () => {
    ...
    try {
      ...
      // Errors handled via global store error state.
    }
  };
  ...
  const applyCSVMapping = () => {
    ...
    csvRawData.rows.forEach((row) => {
      ...
      const legacyPrice = csvColumnMapping.price >= 0 ? (parsePriceField(row[csvColumnMapping.price]) ?? 0) : 0;
      newItems.push({
  ...
  const confirmCSVImport = async () => {
    try {
      ...
      const results = await Promise.allSettled(sanitizedItems.map((item) => addInventoryItem(item)));
      ...
      if (failed > 0 && succeeded === 0) {
        ...
        console.error('Import failures:', results.filter((r) => r.status === 'rejected').map((r) => (r as PromiseRejectedResult).reason));
      }
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {csvPreviewItems.map((item, index) => (
                      <tr key={index} className={`hover:bg-slate-50 transition-colors ${csvSelectedRows.has(index) ? 'bg-blue-50' : ''}`}>
        ...
        <Suspense fallback={<DeferredContentFallback label="Loading stock transfer..." />}>
          <StockTransferModal
            ...
            onTransfer={async (itemId, fromLocationId, toLocationId, quantity, reason) => {
              try {
                await createStockTransfer({ itemId, fromLocationId, toLocationId, quantity, reason });
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={itemToEdit.name} onChange={(e) => setItemToEdit({ ...itemToEdit, name: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Item name" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Category *</label>
                  <input type="text" value={itemToEdit.category} onChange={(e) => setItemToEdit({ ...itemToEdit, category: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Category" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Price ($)</label>
                  <input type="number" step="0.01" value={itemToEdit.price} onChange={(e) => setItemToEdit({ ...itemToEdit, price: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="0.00" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Reorder Level</label>
                  <input type="number" value={itemToEdit.reorderLevel} onChange={(e) => setItemToEdit({ ...itemToEdit, reorderLevel: parseInt(e.target.value) || 0 })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="5" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Supplier</label>
                  <select value={itemToEdit.supplierId} onChange={(e) => setItemToEdit({ ...itemToEdit, supplierId: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl bg-white shadow-sm">
                ...
                <div>
                  ...
                  <input type="text" value={itemToEdit.supplierCode} onChange={(e) => setItemToEdit({ ...itemToEdit, supplierCode: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Product code" />
                </div>
              </div>
            </div>
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex space-x-3">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={itemToEdit.name} onChange={(e) => setItemToEdit({ ...itemToEdit, name: e.target.value })} className="w-full px-4 py-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Item name" />
                </div>
                <div>
                  ...
                  <input type="text" value={itemToEdit.category} onChange={(e) => setItemToEdit({ ...itemToEdit, category: e.target.value })} className="w-full px-4 py-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Category" />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="number" step="0.01" value={itemToEdit.price} onChange={(e) => setItemToEdit({ ...itemToEdit, price: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="0.00" />
                </div>
                <div>
                  ...
                  <input type="number" value={itemToEdit.quantity} onChange={(e) => setItemToEdit({ ...itemToEdit, quantity: parseInt(e.target.value) || 0 })} className="w-full px-4 py-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="0" />
                </div>
                <div>
                  ...
                  <input type="number" value={itemToEdit.reorderLevel} onChange={(e) => setItemToEdit({ ...itemToEdit, reorderLevel: parseInt(e.target.value) || 0 })} className="w-full px-4 py-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="5" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={itemToEdit.supplierId} onChange={(e) => setItemToEdit({ ...itemToEdit, supplierId: e.target.value })} className="w-full px-4 py-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-xl shadow-sm">
                    <option value="">Select supplier...</option>
                    {contacts.filter((c) => c.type === 'Supplier').map((supplier) => (
                      <option key={supplier.id} value={supplier.id}>{supplier.name}</option>
                    ))}
                ...
                <div>
                  ...
                  <input type="text" value={itemToEdit.supplierCode} onChange={(e) => setItemToEdit({ ...itemToEdit, supplierCode: e.target.value })} className="w-full px-4 py-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Product code" />
                </div>
              </div>
              <div>
                ...
                <input type="text" value={itemToEdit.location} onChange={(e) => setItemToEdit({ ...itemToEdit, location: e.target.value })} className="w-full px-4 py-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Warehouse location (e.g., Aisle 3, Shelf B)" />
              </div>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={contactToEdit.name} onChange={(e) => setContactToEdit({ ...contactToEdit, name: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Contact name" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Company</label>
                  <input type="text" value={contactToEdit.company || ''} onChange={(e) => setContactToEdit({ ...contactToEdit, company: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Company name" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Email *</label>
                  <input type="email" value={contactToEdit.email} onChange={(e) => setContactToEdit({ ...contactToEdit, email: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="email@example.com" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Phone</label>
                  <input type="tel" value={contactToEdit.phone} onChange={(e) => setContactToEdit({ ...contactToEdit, phone: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="(555) 123-4567" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Type</label>
                <select value={contactToEdit.type} onChange={(e) => setContactToEdit({ ...contactToEdit, type: e.target.value as 'Supplier' | 'Plumber' | 'Customer' })} className="w-full px-4 py-3 border border-slate-200 rounded-xl bg-white shadow-sm">
  ...
  const state: AppModalsState = {
    ...
    setShowAIAssistant,
    isMobileStockCountOpen,
    setIsMobileStockCountOpen,
  };

  const handlers: AppModalsHandlers = {
    handleConfirmPick,
    handleImportCSV,
    handleDeleteItem,
    ...
    handleMobileStockUpdate,
    getAllocatedItemsFromSelection,
    getStockSourceName,
  };

  return { state, handlers, modalsJSX };
}
