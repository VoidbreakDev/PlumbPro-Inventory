...
async function runAllMigrations() {
  ...
  try {
    ...
    if (existingTables.rows[0].count > 0 && process.env.ALLOW_DESTRUCTIVE_RESET !== 'true') {
      throw new Error(
        'Refusing to replay destructive schema files against a non-empty database. Use `npm run migrate` for additive upgrades or set ALLOW_DESTRUCTIVE_RESET=true to force a reset.'
      );
    }
