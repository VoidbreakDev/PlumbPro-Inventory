...
async function seedDatabase() {
  // SECURITY: Prevent demo data from being seeded in production
  if (process.env.NODE_ENV === 'production') {
    console.error('❌ SECURITY WARNING: Cannot run seed script in production environment!');
    console.error('   Demo credentials should never exist in production databases.');
    console.error('   If you need to create an admin user, use a secure user creation script.');
    process.exit(1);
  }
  ...
  try {
    console.log('🌱 Starting database seeding (DEVELOPMENT MODE ONLY)...');
    console.warn('⚠️  WARNING: This creates demo accounts with weak passwords.');
