...
import { TrendingUp, AlertTriangle, Calendar, Users, ClipboardList, BarChart3, LayoutDashboard } from 'lucide-react';
...
const EnhancedDashboardView = React.lazy(async () => {
  const module = await import('./EnhancedDashboardView');
  return { default: module.EnhancedDashboardView };
});
...
export const DashboardView: React.FC<DashboardViewProps> = ({ inventory, jobs, contacts, onNavigate }) => {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Dashboard Modes</h2>
          <p className="mt-1 text-sm text-slate-500">
            Use Overview for quick operational status, or switch to Analytics for deeper movement and job trends.
          </p>
        </div>
      ...
      {activeView === 'analytics' ? (
        <Suspense fallback={<DeferredContentFallback label="Loading dashboard analytics..." />}>
          <EnhancedDashboardView embedded onNavigate={onNavigate} />
        </Suspense>
      ) : (
        <>
